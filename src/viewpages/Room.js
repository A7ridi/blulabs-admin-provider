...
const Room = ({ roomName, token, handleLogout }) => {
  ...
  useEffect(() => {
    ...
    async function doSubmit() {
      ...
      const participantDisconnected = (participant) => {
        ...
        handleLogout(callConnection);
      };
      setlocalVideoTrack(await Video.createLocalVideoTrack());
  ...
  const callNotReceived = useCallback((event) => {
    if (callConnection) {
      swal(
        localStorage.getItem("patientNameVideo") +
          " didn't accept your call, please try after some time."
      ).then((value) => {
  ...
  const getUserScreen = async () => {
    //var extensionId = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    if (!canScreenShare()) {
      return;
    }
    if (isChrome()) {
      try {
        ...
        if (!screenTrack) {
          ...
          //screenTrackNew = new Video.localVideoTrack(newScreenTrack)

          // this.setState({
          //     screenTrack: new Video.localVideoTrack(newScreenTrack)
          // });
          document.getElementById("button-share-screen-test").style.display =
            "none";
          document.getElementById("button-unshare-screen-test").style.display =
            "inline";
  ...
  const hideBeforeCall = (status) => {
    if (status) {
      document.getElementById("button-share-screen-before").style.display =
        "none";
      ...
      {callConnection ? (
        <div className="mute-dis-buttton">
          <button className="share-btn">
            <img
              id="button-share-screen-before"
              onClick={() => hideBeforeCall(true)}
              alt=""
              src="/assets/images/Share.svg"
              title="Share Screen"
            ></img>
            <img
              id="button-unshare-before"
              style={{ display: "none" }}
              alt=""
              onClick={() => hideBeforeCall(false)}
              src="/assets/images/Stop-Share.svg"
              title="Stop Sharing"
            ></img>
          </button>
          {mikeVoice ? (
          ...
          <button className="close" title="Disconnect" onClick={handleLogout}>
            Disconnet
          </button>
        </div>
      ) : (
        <div className="mute-dis-buttton">
          {browserName ? (
            <React.Fragment>
              ...
              {/* <button id="button-unshare-screen" style={{ display: "none" }}>Stop Sharing</button> */}
            </React.Fragment>
          ) : (
            <React.Fragment>
              <button className="share-btn">
                <img
                  id="button-share-screen-test"
                ...
                ></img>
                <img
                  id="button-unshare-screen-test"
                  onClick={() => stopTrack("screenTrack")}
                  style={{ display: "none" }}
                  src="/assets/images/Stop-Share.svg"
                  title="Stop Sharing"
                  alt=""
                ></img>
                {/* <button id="button-share-screen-test" onClick={() => getUserScreen()}>Share Screen</button>
                            <button id="button-unshare-screen-test" onClick={() => stopTrack("screenTrack")} style={{ display: "none" }}>Stop Sharing</button> */}
              </button>
            </React.Fragment>
          )}
          {mikeVoice ? (
